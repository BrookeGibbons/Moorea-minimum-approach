...
hab <- read.delim("Data/Habitat/170421_inside_outside_habitat.TXT", header = T, skip = 4, stringsAsFactors = FALSE) %>%
...
fov <- hab %>%
  ...
  filter(!fieldofview %in% c("", NA, NULL)) %>%
...
broad <- hab %>%
  ...
  mutate(broad = ifelse(broad == "Octocoral/Black", "Stony.corals",
                        ifelse(broad == "Stony corals", "Stony.corals",
                               ifelse(broad == "Open Water", "Open.Water", broad)))) %>% #correct bad names
  filter(!broad == "") %>%
  filter(!is.na(broad)) %>%
...
habitat <- relief %>%
  full_join(fov, by = "Sample") %>%
  full_join(broad, by = "Sample") %>%
  rename(hard.corals = broad.Stony.corals, 
...
write.csv(habitat, paste("Data/Tidy data/", study, "_habitat.csv", sep = ""), row.names = FALSE)
saveRDS(habitat, "Data/Tidy data/habitat.RDS")