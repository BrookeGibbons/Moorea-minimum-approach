...
length.factors <- read.csv(paste("Data/Tidy data/", study, ".length.factors.csv", sep = "")) #TJL-Need to add this in

length <- readRDS("Data/Tidy data/length.RDS") %>%
...
saveRDS(length.expanded.factors, "Data/Tidy data/length.expanded.factors.RDS")

# Bring in the enviromental data----
dov.fine.habitat <- readRDS("Data/Tidy data/habitat.RDS") %>%
  glimpse()

# Bring in period time ----
period.times <- read.delim("Data/EM export/period.lengths.TXT", header = T, skip = 4, stringsAsFactors = FALSE) %>%
...
final.data <- data %>%
  ...
  mutate(Region = ifelse(Region == "East", "Northeast",
                         ifelse(Region == "North", "Northeast", Region))) %>%
...
# one is a ray and one is on the reef slope, so I think it is fine for me to delete

# write.csv(final.data, file=paste(Sys.Date(),study,"combined.factors.habitat.csv",sep = "_"), row.names=FALSE)
saveRDS(final.data, "Data/Tidy data/length.combined.factors.habitat.RDS")
